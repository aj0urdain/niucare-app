...
export function ForgotPasswordForm() {
  ...
  const onEmailSubmit = async (data: z.infer<typeof forgotPasswordSchema>) => {
    try {
      ...
      router.push(`?mode=reset&email=${encodeURIComponent(data.email)}`);
    } catch (err: any) {
    ...
    if (mode === "reset") {
      ...
      setTimeout(() => {
        ...
        if (otpRef.current) {
          ...
          const el = document.querySelector('[data-slot="input-otp-slot"] input') as HTMLInputElement | null;
          el?.focus();
        }
      }, 0);
    }
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className={cn("space-y-1")}> 
                    <FormLabel className={cn("text-xs font-normal opacity-60 ml-1")}> 
                      Email
                    </FormLabel>
                    <FormControl>
                      <Input
